...
exports.DevQueryBuilder = exports.__devStore = exports.emptyStore = void 0;
...
function createStore() {
    const store = (0, exports.emptyStore)();
    seedStore(store);
    return store;
}
/** 현재 전역 스토어 (테스트에서 초기화/검증용) */
exports.__devStore = { store: null };
function getStore() {
    if (!exports.__devStore.store)
        exports.__devStore.store = createStore();
    return exports.__devStore.store;
}
function resetStore() {
    exports.__devStore.store = null;
    return getStore();
...
class DevQueryBuilder {
    ...
    execSelect(op, scope) {
        ...
        for (const f of op.filters)
            rows = rows.filter((r) => f.predicate(r[f.field]));
        if (op.orderBy) {
...
exports.DevQueryBuilder = DevQueryBuilder;
function createDevClient(store) {
    const client = {
        ...
        rpc: async (fn, args = {}) => {
            ...
            if (fn === 'increment_neuron_usage') {
                ...
                if (neuron) {
                    ...
                    if (args.p_success)
                        neuron.success_count += 1;
                    else
                        neuron.failure_count += 1;
                }
                return { data: null, error: null };
            }
